...
echo "Marking vms for model $model as managed=false"
jq "(.catalog[].servers)|=(map((if .name|test(\"^juju-[a-z0-9]+-${model}-[0-9]+\") then .managed=\"false\" else . end)))" $STAGING_DIR/managed_vms.json > $STAGING_DIR/managed_vms.json.updated
