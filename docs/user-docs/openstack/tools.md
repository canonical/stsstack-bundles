...
Create a load balancer with Octavia. This command assumes that Octavia has been properly configures, see [`configure_octavia.sh`](#configure-octavia-sh) and `upload_octavia_amphora_image.sh` for details.

### Example

```console
configure_octavia.sh --name lb \
    --member-vm server-1 \
    --provider amphora \
    --protocol TCP \
    --protocol-port 22
...
Installs the CA for ssl locally. See [`generate-bundles.sh`](../generate-bundle)
